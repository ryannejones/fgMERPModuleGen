...
class BattleGenerator:
    ...
    def get_creature_link(self, creature_name):
        ...
        if self.library.creature_exists(creature_name):
            # Library creatures reference Character Law
            # We'll use a simplified reference for now
            return {
                'class': 'npc',
                'recordname': f'reference.npcs.{creature_name.lower().replace(" ", "")}@Character Law'
            }
        
        # Custom NPC - will reference the custom NPCs section we'll generate
        # For now, use a placeholder
        return {
            'class': 'npc', 
            'recordname': f'npc.{creature_name.lower().replace(" ", "_")}'
        }
