...
def main():
    ...
    try:
        ...
        if args.verbose:
            print(f"  Loaded {stats['npcs_count']} NPCs/creatures")
            print(f"  Loaded {stats['items_count']} items")
            print(f"  Loaded {stats['skills_count']} skills")
            print(f"  Loaded {stats['spell_lists_count']} spell lists")
        print("  [OK] Reference library loaded")
    except Exception as e:
        print(f"\nERROR: Failed to load reference library: {e}")
        if args.verbose:
            import traceback
            traceback.print_exc()
        return 1
    ...
    # Phase 4: Generate Items XML (MUST be first - NPCs and Parcels reference items)
    ...
    # Phase 5: Generate NPC XML (needs Items for weapon references)
    print("Phase 5: Generating NPC XML")
    print("-" * 60)
    npc_gen = NPCGenerator(loader, library, verbose=args.verbose)
    npc_xml = npc_gen.generate()
    
    if npc_xml is not None:
        # Count custom + encounter NPCs
        npc_count = len([child for child in npc_xml])
        print(f"  [OK] Generated {npc_count} NPCs")
    else:
        print("  [SKIP] No NPCs to generate")
    print()
    
    # Phase 6: Generate Battle XML (needs NPCs for ID references)
    print("Phase 6: Generating Battle XML")
    print("-" * 60)
    ...
    # Phase 7: Generate Treasure Parcels XML (needs Items for references)
    print("Phase 7: Generating Treasure Parcels XML")
    print("-" * 60)
    ...
    # Phase 8: Generate Story XML (can reference NPCs, Items, Battles)
    print("Phase 8: Generating Story XML")
    print("-" * 60)
    ...
    # Phase 9: Generate Images XML
    print("Phase 9: Generating Images XML")
    print("-" * 60)
